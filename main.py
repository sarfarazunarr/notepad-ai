...
class Notepad:
    ...
    def use_ai(self):
        ...
        prompt_popup.geometry("300x150")
        prompt_popup.resizable(False, False)
        prompt_popup.config(bg="#f0f0f0")
        prompt_popup.attributes("-topmost", True)

        prompt_label = tk.Label(prompt_popup, text="Enter your prompt:", bg="#f0f0f0", fg="#333", font=("Arial", 12))
        prompt_label.pack(padx=20, pady=20)

        prompt_entry = tk.Entry(prompt_popup, width=30, font=("Arial", 12))
        prompt_entry.pack(padx=20, pady=10)
        ...
        ok_button = tk.Button(prompt_popup, text="OK", command=get_prompt, bg="#4CAF50", fg="#fff", font=("Arial", 12), width=10, height=2, relief=tk.RAISED, borderwidth=3)
        ok_button.pack(pady=20)
        ok_button.bind("<Enter>", lambda e: ok_button.config(bg="#45a049"))
